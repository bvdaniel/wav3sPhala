...
contract ZurfRound0Vesting {
    ...
    function loadVestingData(address[] calldata investors, uint128[] calldata _zrfTokens,uint40 cliffMonths, uint40 vestingMonths, uint40 vestingMonthsLargeInvestor, bool _startAt0) external onlyOwner onlyZurfTrigger {
       require(investors.length == _zrfTokens.length, "Array lengths must match");
        ...
        for (uint256 i; i < investors.length; i++) {
            ...
            if(_zrfTokens[i] >= 5e24){
                allocations[investors[i]].vestedDuration = vestingMonthsLargeInvestor;
                } else{
