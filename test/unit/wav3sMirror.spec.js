...
!developmentChains.includes(network.name)
    ? describe.skip
    : describe("wav3sMirror Unit Tests", async function () {
          ...
          let wav3s_fee = 5
          let base_fee = BigNumber.from(10).pow(18).mul(1)


          // We define a fixture to reuse the same setup in every test.
          // We use loadFixture to run this setup once, snapshot that state,
          // and reset Hardhat Network to that snapshot in every test.
          async function deploywav3sMirrorFixture() {
              const [deployer] = await ethers.getSigners()
              accounts = await ethers.getSigners() // could also do with getNamedAccounts

              anyProfileId = accounts[2]

              const wav3sMirrorFactory = await ethers.getContractFactory("wav3sMirror")

              const wav3sMirror = await wav3sMirrorFactory
                  .connect(deployer)
                  .deploy(wav3s_fee/*, wEther, USDCoin, DAI*/)
              return { wav3sMirror, USDT,WMATIC }
          }
          // Fixture correctly set up
          async function deploywav3sMirrorFixtureSetup() {
              const [deployer] = await ethers.getSigners()
              ...
              const wav3sMirror = await wav3sMirrorFactory
                  .connect(deployer)
                  .deploy(wav3s_fee/*, wEther, USDCoin , DAI*/)
              await wav3sMirror.setMultisig(multisig)
              await wav3sMirror.whitelistWav3sTrigger(WAV3S_HUB)
              await wav3sMirror.whitelistCurrency(WMATIC, false)
              ...
              const wav3sPreBalance = await Currency.balanceOf(wav3sMirror.address)
          ...
          async function deploywav3sMirrorFixturePubFunded() {
            const [deployer] = await ethers.getSigners()
            accounts = await ethers.getSigners(1) // could also do with getNamedAccounts
            ...
            await wav3sMirror.setMultisig(multisig)
            await wav3sMirror.whitelistWav3sTrigger(WAV3S_HUB)
            await wav3sMirror.whitelistCurrency(WMATIC)
            ...
            let isWav3 = await wav3sMirror.isWav3(pubId)
            ...
            await expect(
                ...
                .to.emit(wav3sMirror, "wav3sMirror__PostFunded")
                .withArgs(
                  budget.toString(),
                  reward.toString(),
                  profileAddress,
               ...
               // console.log("aqui22")
                tx = await wav3sMirror.connect(wav3sHub).setPubId(1,pubId)
            ...
            wav3spostOwnerBalance = await Currency.balanceOf(wav3sMirror.address)
            ...
            assert.equal(
                ...
                "Post wav3sMirror balance should be equal to pre balance plus preBudget minus wav3sFees"
            )
            ...
            // Check isWav3 works
            isWav3 = await wav3sMirror.isWav3(pubId)
            expect(isWav3).to.equal(true)
            // Check getMirrorBudget works

            mirrorBudget = await wav3sMirror.getMirrorBudget(pubId)
          ...
          async function deploywav3sMirrorFixturePubFundedLittleBudget() {
              const [deployer] = await ethers.getSigners()
              accounts = await ethers.getSigners(1) // could also do with getNamedAccounts
            ...
            await expect(
              ...
              // check that wav3s balance is preWav3sBalance, plus the budget, minus the wav3s fees
              wav3sPostBalance = await Currency.balanceOf(wav3sMirror.address)
              ...
              const preWav3sBalance = await Currency.balanceOf(wav3sMirror.address)
              ...
              await expect(
                  ...
                  .to.emit(wav3sMirror, "wav3sMirror__MirrorProcessed")
                  .withArgs(
                  ...
                  .to.emit(wav3sMirror, "wav3sMirror__PubFinished")
                  .withArgs(pubId)
              ...
              postWav3sBalance = await Currency.balanceOf(wav3sMirror.address)

              expectedWav3sBalance = preWav3sBalance.sub(reward)
              assert.equal(
                  postWav3sBalance.toString(),
                  expectedWav3sBalance.toString(),
                  "Post wav3sMirror balance should be equal to pre balance minus reward"
              )
          ...
          describe("Set up hub, whitelist app", async function () {
              describe("Set Hub", async function () {
                  it("Should emit event wav3sMirror__HubSet when setting wav3s hub address", async function () {
                      const { wav3sMirror } = await loadFixture(deploywav3sMirrorFixture)
                      const deployer = await wav3sMirror.signer
                      const newWav3sHub = "0x0000000000000000000000000000000000000001"

                      await expect(wav3sMirror.whitelistWav3sTrigger(newWav3sHub)).not.to.be.reverted
                      await expect(wav3sMirror.whitelistWav3sTrigger(newWav3sHub))
                          .to.emit(wav3sMirror, "wav3sMirror__TriggerSet")
                  ...
                  it("Should revert when setting wav3s hub address with non owner", async function () {
                      ...
                      await expect(
                          wav3sMirror.connect(anyProfileId).whitelistWav3sTrigger(newWav3sHub)
              ...
              describe("Set Multisig", async function () {
                  it("Should emit event wav3sMirror__HubSet when setting wav3s hub address", async function () {
                      ...
                      await expect(wav3sMirror.setMultisig(multisig)).not.to.be.reverted
                      await expect(wav3sMirror.setMultisig(multisig))
                          .to.emit(wav3sMirror, "wav3sMirror__MsigSet")
                          .withArgs(multisig, deployer.address)
                      const multisigAddress = await wav3sMirror.getMultisig()
                  ...
                  it("Should revert when setting multisig address with non owner", async function () {
                      ...
                      await expect(
                          wav3sMirror.connect(anyProfileId).setMultisig(multisig)
                      ).to.be.rejectedWith("Only the owner can call this function.")
                  })
              })
              describe("Whitelist currency", async function () {
                  it("Should emit event when normal currency is whitelisted and currencyWhitelisted return (currency,false)", async function () {
                      const { wav3sMirror, WMATIC } = await loadFixture(deploywav3sMirrorFixture)

                      await expect(wav3sMirror.whitelistCurrency(WMATIC, false)).not.to.be.reverted
                      await expect(wav3sMirror.whitelistCurrency(WMATIC, false))
                          .to.emit(wav3sMirror, "wav3sMirror__CurrencyWhitelisted")
                          .withArgs(WMATIC, false)

                      const isWhitelisted = await wav3sMirror.currencyWhitelisted(WMATIC)
                  ...
                  it("Should emit event when super currency is whitelisted and currencyWhitelisted return (currency,true)", async function () {
                    const { wav3sMirror, USDT } = await loadFixture(deploywav3sMirrorFixture)

                    await expect(wav3sMirror.whitelistCurrency(USDT, true)).not.to.be.reverted
                    await expect(wav3sMirror.whitelistCurrency(USDT, true))
                        .to.emit(wav3sMirror, "wav3sMirror__CurrencyWhitelisted")
                        .withArgs(USDT, true)

                    const isWhitelisted = await wav3sMirror.superCurrencyWhitelisted(USDT)
                    expect(isWhitelisted).to.equal(true)
                })
                  it("Should emit error not owner and superCurrencyWhitelisted return false", async function () {
                      ...
                      await expect(
                          wav3sMirror.connect(anyProfileId).whitelistCurrency(USDT, true)
                      ).to.be.rejectedWith("Only the owner can call this function.")
                      const isWhitelisted = await wav3sMirror.superCurrencyWhitelisted(USDT)
              ...
              describe("Fund a post", async function () {
                  it("Should revert sender not owner", async function () {
                      const { wav3sMirror,currencyAddress ,WHALE_USER } =
                          await loadFixture(deploywav3sMirrorFixtureSetup)
                     ...
                     // let pubId = "0x1d"
                      ...
                      await expect(
                          ...
                          "Errors.wav3sMirror__fundMirror__SenderNotOwner()"
                      )
                  })
                  it("Should revert if reward is below minimum", async function () {
                    const { wav3sMirror, currencyAddress, Whale } =
                        await loadFixture(deploywav3sMirrorFixtureSetup);
                    ...
                    await expect(
                        ...
                        "Errors.wav3sMirror__fundMirror__RewardBelowMinimum()"
                    );
                })
                  ...
                  it("Should revert if budget is less than the reward", async function () {
                      ...
                      await expect(
                        ...
                        "Errors.wav3sMirror__fundMirror__NotEnoughBudgetForThatReward()"
                        )
                  })
                ...
                console.log("aqui1")
                  ...
                  it("Should revert post already set", async function () {
                      const {
                          ...
                          wav3sHub,
                          pubId                       
                      } = await loadFixture(deploywav3sMirrorFixturePubFunded)

                     await expect(wav3sMirror.connect(wav3sHub).setPubId(1,pubId)).to.be.rejectedWith(
                            "Errors.wav3sMirror__setPubId__PostAlreadyFunded/Set()"
                     )
                  })
                      ...
                      it("Should fundMirror succesfully", async function () {
                        ...
                        let isWav3 = await wav3sMirror.isWav3(pubId)
                          ...
                          .fundMirror(
                              ...
                              profileAddress,
                              minFollowers,
                              currencyAddress)).to.emit(wav3sMirror, "wav3sMirror__PostFunded")
                          .withArgs(
                            budget.toString(),
                            reward.toString(),
                            profileAddress,
                           ...
                           assert.equal(
                               wav3spostOwnerBalance.toString(),
                               expectedWav3sBalance.toString(),
                               "Post wav3sMirror balance should be equal to pre balance plus preBudget minus wav3sFees, minus consumerAppFees"
                           )
                           ...
                           // pubid por hash
                           isWav3 = await wav3sMirror.isWav3(0)
                           expect(isWav3).to.equal(false)
                            // pubid por pubid
                            isWav31 = await wav3sMirror.isWav3(1)
                            expect(isWav3).to.equal(false)
                           // Check getMirrorBudget works
 
                           mirrorBudget = await wav3sMirror.getMirrorBudget(0)
                           expect(mirrorBudget).to.equal(0)

                           // set pubid
                           // not working
                           tx = await wav3sMirror.connect(wav3sHub).setPubId(0,pubId)
                           expect(tx).not.to.be.reverted

                            // pubid por hash
                            isWav32 = await wav3sMirror.isWav3(0)
                            expect(isWav3).to.equal(false)
                            // Check getMirrorBudget works
                            // Check isWav3 works
                           ...
                           //mirrorBudget = await wav3sMirror.getMirrorBudget(socialGraph,pubId)
                           //expect(mirrorBudget).to.equal(budget.toString())
                      })
                  })
              })
              describe("Process a Mirror", async function () {
                  it("Fails if pub is not started", async function () {
                      const { wav3sMirror, wav3sHub, currencyAddress, pubId } =
                          await loadFixture(deploywav3sMirrorFixtureSetup)
                      await expect(
                          wav3sMirror
                              .connect(wav3sHub)
                  ...
                  it("Fails if its not wav3sHub calling", async function () {
                      const { wav3sMirror, currencyAddress, pubId, anyProfileId } =
                          await loadFixture(deploywav3sMirrorFixturePubFunded)
                      await expect(
                          wav3sMirror
                  ...
                  it("Should revert follower already mirrored", async function () {
                      const {
                          ...
                          currencyAddress,
                          appAddress,
                      } = await loadFixture(deploywav3sMirrorFixturePubFundedLittleBudget)
                      await expect(
                          wav3sMirror
                              .connect(wav3sHub)
                              .processMirror(pubId, anyProfileId.address.toString(), 10)
                      ).to.be.rejectedWith(
                        "Errors.wav3sMirror__process__FollowerAlreadyMirrored()"
                        )
                  })
                  it("Should revert not enough budget", async function () {
                      const { wav3sMirror, feePerMirror, Whale, pubId, wav3sHub, appAddress } =
                          await loadFixture(deploywav3sMirrorFixturePubFundedLittleBudget)
                      await expect(
                          wav3sMirror
                              .connect(wav3sHub)
                        ...
                        "Errors.wav3sMirror__process__NotEnoughBudgetForThatReward()"
                        )
                  })
                  it("Should revert not enough followers", async function () {
                      ...
                      await expect(
                          ...
                          "Errors.wav3sMirror__process__NeedMoreFollowers()"
                      )
                  })
              ...
              describe("Process a Withdraw", async function () {
                  it("Should revert sender not owner", async function () {
                      const { wav3sMirror, pubId, anyProfileId } = await loadFixture(
                          deploywav3sMirrorFixturePubFunded
                      )

                      // should emit Events.wav3sMirror__PubWithdrawn(pubId, msg.sender);
                      await expect(
                          wav3sMirror.connect(anyProfileId).withdrawMirrorBudget(pubId)
                      ).to.be.rejectedWith(
                        "Errors.wav3sMirror__withdraw__NotSenderProfileToWithdraw()"
                        )
                  })

                  it("Should revert invalid pubId", async function () {
                      const { wav3sMirror, Whale, pubId } = await loadFixture(
                          deploywav3sMirrorFixturePubFunded
                      )
                      // should emit Events.wav3sMirror__PubWithdrawn(pubId, msg.sender);
                      await expect(
                          wav3sMirror.connect(Whale).withdrawMirrorBudget("")
                      ).to.be.rejectedWith(
                          "Errors.wav3sMirror__withdraw__InvalidPubId()"
                      )
                  })

                  it("Should revert wav3 not initiated", async function () {
                      const { wav3sMirror, Whale, pubId } = await loadFixture(
                          deploywav3sMirrorFixtureSetup
                      )
                      await expect(
                          wav3sMirror.connect(Whale).withdrawMirrorBudget(pubId)
                      ).to.be.rejectedWith(
                        "Errors.wav3sMirror__withdraw__PostNotInitiated()"
                        )
                  })
                  it("Should revert no budget to withdraw", async function () {
                      const { wav3sMirror, Whale, budget, pubId, profileAddress } =
                          await loadFixture(deploywav3sMirrorFixturePubFundedLittleBudget)
                      await expect(
                          wav3sMirror.connect(Whale).withdrawMirrorBudget(pubId)
                      ).to.be.rejectedWith(
                        "Errors.wav3sMirror__withdraw__BudgetEmpty()"
                        )
                  })
                  describe("Success", async function () {
                      it("Should withdraw succesfully", async function () {
                          ...
                          preWav3sBalance = await Currency.balanceOf(wav3sMirror.address)

                          // Should allow profile to withdraw funds
                          // should emit Events.wav3sMirror__PubWithdrawn(pubId, msg.sender);
                          await expect(wav3sMirror.connect(Whale).withdrawMirrorBudget(pubId))
                              .to.emit(wav3sMirror, "wav3sMirror__PubWithdrawn")
                          ...
                          // balance of wav3sMirror should decrease in the budget amount
                          let expectedWav3sBalance = preWav3sBalance.sub(budget.toString())
                          assert.equal(
                              postWav3sBalance.toString(),
                              expectedWav3sBalance.toString(),
                              "Post wav3sMirror balance should be equal to pre balance minus mirrorBudget"
                          )
                          ...
                          let postPubData = await wav3sMirror.getPubData(pubId)
              ...
              describe("Circuit braker", async function () {
                  describe("Should not allow", async function () {
                      ...
                      it("Should revert not owner", async function () {
                          const { wav3sMirror, anyProfileId } = await loadFixture(
                              deploywav3sMirrorFixtureSetup
                          )
                          await expect(
                              wav3sMirror.connect(anyProfileId).circuitBreaker()
                          ).to.be.rejectedWith("Only the owner can call this function.")
                      })
                      it("Should not allow to fund", async function () {
                          ...
                          await expect(wav3sMirror.circuitBreaker())
                              .to.emit(wav3sMirror, "wav3sMirror__CircuitBreak")
                              .withArgs(true)
                      ...
                      it("Should not allow to processMirror", async function () {
                          const { wav3sMirror, wav3sHub, pubId, anyProfileId } =
                              await loadFixture(deploywav3sMirrorFixtureSetup)
                      ...
                      it("Should not allow to Withdraw ", async function () {
                          const { wav3sMirror, Whale, pubId, anyProfileId } = await loadFixture(
                              deploywav3sMirrorFixtureSetup
                          )
                  ...
                  describe("EmergencyWithdraw", async function () {
                      it("Should revert invalid pubId", async function () {
                          const { wav3sMirror, wav3sHub, pubId } = await loadFixture(
                              deploywav3sMirrorFixturePubFunded
                          )
                          await expect(wav3sMirror.circuitBreaker())
                              .to.emit(wav3sMirror, "wav3sMirror__CircuitBreak")
                          ...
                          // should emit Events.wav3sMirror__PubWithdrawn(pubId, msg.sender);
                          await expect(
                              wav3sMirror.connect(wav3sHub).withdrawPub("")
                          ).to.be.rejectedWith(
                              "Errors.wav3sMirror__EmergencyWithdraw__InvalidPubId()"
                          )
                      })
                      ...
                      it("Should revert wav3 not initiated", async function () {
                          ...
                          await expect(
                              wav3sMirror.connect(wav3sHub).withdrawPub(pubId)
                          ).to.be.rejectedWith(
                              "Errors.wav3sMirror__EmergencyWithdraw__Wav3NotInitiated()"
                          )
                      })
                      it("Should revert no budget to withdraw", async function () {
                          const { wav3sMirror, wav3sHub, pubId } = await loadFixture(
                              deploywav3sMirrorFixturePubFundedLittleBudget
                          )
                          ...
                          await expect(
                              ...
                              "Errors.wav3sMirror__EmergencyWithdraw__NotEnoughBudgetToWithdraw()"
                          )
                      })
                      describe("Success", async function () {
                          it("Should allow withdrawing pub in emergency to wav3sHub", async function () {
                              ...
                              await expect(wav3sMirror.connect(wav3sHub).withdrawPub(pubId))
                                  .to.emit(wav3sMirror, "wav3sMirror__EmergencyWithdraw")
                  ...
                  describe("Backdoor", async function () {
                      it("Should revert not in emergency", async function () {
                          ...
                          await expect(
                              wav3sMirror.connect(wav3sHub).backdoor()
                      ...
                      it("Should revert not owner", async function () {
                          ...
                          await expect(wav3sMirror.circuitBreaker())
                              ...
                              .withArgs(true)
                          await expect(
                              wav3sMirror.connect(anyProfileId).backdoor()
                          ).to.be.rejectedWith("Only the owner can call this function.")
                      })
                      describe("Succes", async function () {
                          it("Should transfer funds to owner", async function () {
                              ...
                              await expect(wav3sMirror.backdoor())
                                  .to.emit(wav3sMirror, "wav3sMirror__backdoor")
                                  .withArgs(preWav3sBalance)

                              postWav3sBalance = await Currency.balanceOf(wav3sMirror.address)
                              postOwnerBalance = await Currency.balanceOf(deployer.address)

                              // balance of wav3sMirror should be 0
                              let expectedWav3sBalance = 0
                              assert.equal(
                                  postWav3sBalance.toString(),
                                  expectedWav3sBalance.toString(),
                                  "Post wav3sMirror balance should be equal to pre balance minus postAppFees"
                              )
                              ...
                              assert.equal(
                                  ...
                                  "Post owner balance should be equal to pre balance plus wav3sMirror balance"
                              )
                          })
                      })
                  })
